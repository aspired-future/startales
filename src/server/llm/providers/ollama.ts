...
export class OllamaProvider implements LLMProvider {
  ...
  async complete(messages: ModelMessage[], options?: CompletionOptions): Promise<CompletionResponse> {
    ...
    if (!resp.ok) throw new Error(`ollama_chat_failed: ${resp.status}`)
  ...
  async *completeStream(messages: ModelMessage[], options?: CompletionOptions): AsyncIterable<StreamChunk> {
    ...
    if (!resp.ok || !resp.body) throw new Error(`ollama_stream_failed: ${resp.status}`)
    ...
    while (true) {
      ...
      for (const line of chunk.split('\n')) {
        ...
        try {
          ...
          if (evt?.message?.content) yield { type: 'text', deltaText: evt.message.content }
          if (evt?.done) yield { type: 'end' }
        } catch {
