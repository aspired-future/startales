export interface OutcomePreviewInput {
  dc: number
  attribute: number
  skill: number
  modifiers?: number
  momentum?: number // converts complication to resource cost (simple shift)
  attempts?: number // for pity scaling
}

export interface OutcomeBands {
  fail: number // probability 0..1
  complication: number
  success: number
  critical: number
}
...
export interface OutcomePreview {
  ...
  expectedCost: number // abstract unit
  notes?: string[]
}

/** Simple band calculator for Sprint 1 */
export function previewOutcome(input: OutcomePreviewInput): OutcomePreview {
  ...
  let bands: OutcomeBands = { fail: 0.25, complication: 0.25, success: 0.4, critical: 0.1 }
  ...
  const notes: string[] = []
  if (pityBonus > 0) notes.push(`pity_bonus:+${pityBonus}`)
  if (momentum > 0) notes.push(`momentum_convert:${(convert * 100).toFixed(0)}%`)
