...
const db: { campaigns: Campaign[] } = { campaigns: [] };
...
campaignsRouter.post('/', (req, res) => {
  ...
  const campaign: Campaign = { id: nanoid(), createdAt: now, archived: false, ...parsed.data };
...
campaignsRouter.post('/:id/clone', (req, res) => {
  ...
  const clone: Campaign = { ...base, id: nanoid(), title: `${base.title} (Clone)`, createdAt: new Date().toISOString(), archived: false };
