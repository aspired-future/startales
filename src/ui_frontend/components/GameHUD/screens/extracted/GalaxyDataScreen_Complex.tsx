...
const GalaxyDataScreen: React.FC = () => {
  ...
  useEffect(() => {
    const loadMockData = () => {
      try {
        console.log('🌌 GalaxyDataScreen: Loading mock data...');
        setLoading(true);
        setError(null);
        
        // Use mock data since APIs are not available
        setTimeout(() => {
          // Mock galaxy map data
          setGalaxyMap({
            campaignId: 1,
            sectors: [
              {
                ...
                coordinates: { x: 100, y: 150, z: 50 },
                starSystems: [
                  { id: 'sys-1', name: 'Alpha Centauri A', starType: 'G-class', planets: 3, explored: true, controlledBy: 'player' },
                  { id: 'sys-2', name: 'Proxima Centauri', starType: 'M-class', planets: 2, explored: true, controlledBy: 'player' }
                ],
          ...
          setTerritories({
            playerTerritory: {
              ...
              totalPopulation: 2847000000,
              totalGDP: 45600000000000,
              militaryStrength: 85,
              diplomaticInfluence: 73
            },
            civilizations: [
              {
                id: 'civ-1',
                name: 'Zephyrian Empire',
                controlledSystems: 18,
                territorialClaims: [
                  { systemId: 'sys-5', claimStrength: 85, disputed: false, claimDate: '2024-01-15' }
                ],
                diplomaticStatus: 'neutral',
                militaryStrength: 92,
                economicPower: 78,
                technologicalLevel: 88,
                culturalInfluence: 65
              }
            ]
          });
          ...
          setDiscoveries([
            {
              id: 'disc-1',
              name: 'Ancient Hypergate Network',
              type: 'technology',
              location: { systemId: 'sys-1', coordinates: { x: 100, y: 150, z: 50 } },
              discoveryDate: new Date('2024-01-10'),
              significance: 'critical',
              description: 'Discovered remnants of an ancient faster-than-light transportation network.',
              economicValue: 95,
              strategicValue: 98
            }
          ]);
          ...
          setAIRecommendations({
            strategic: [
              { priority: 'high', recommendation: 'Expand to Vega system for rare minerals' },
              { priority: 'medium', recommendation: 'Strengthen diplomatic ties with Zephyrian Empire' }
            ],
            economic: [
              { priority: 'high', recommendation: 'Invest in quantum computing research' }
            ]
          });
          ...
          setSpatialIntelligence({
            threatLevel: 'low',
            knownHostiles: 2,
            patrolRoutes: 15
          });

          setMilitaryIntelligence({
            fleetReadiness: 0.85,
            defensivePositions: 23,
            earlyWarningStatus: 'active'
          });

          setTradeOpportunities({
            activeRoutes: 47,
            potentialRevenue: 2400000000,
            marketAnalysis: {
              emergingMarkets: ['quantum_tech', 'exotic_matter', 'biotech']
            }
          });

          setSensorContacts({
            sensorStatus: {
              operationalSensors: 156,
              degradedSensors: 12,
              offlineSensors: 3,
            ...
            contacts: [
              {
                ...
                classification: 'merchant_vessel'
              }
            ]
          });
    ...
    fetchGalaxyData();
    // Disabled API calls since we're using mock data
    // fetchEnhancedData();
    // fetchSpatialIntelligenceData();
  }, []);

  if (loading) {
    console.log('🌌 GalaxyDataScreen: Still loading...');
    return (
  ...
  if (error) {
    console.log('🌌 GalaxyDataScreen: Error state:', error);
    return (
      <div className="galaxy-data-screen">
  ...
  console.log('🌌 GalaxyDataScreen: Rendering with data:', { galaxyMap, territories, discoveries });
